...
beg = ["знаете в чем разница между говном и краковской колбасой? ","встретились как-то два деревенских дурочка ",
       ...
       "вы знаете настоящую причину самоубийства гитлера: ","Отец мне говорил: ","забавный случай в детдоме: ","знаете, какой анекдот кулял среди солдат СС: ","Хрущёв часто шутил на своих выступлениях, но эта речь запомнилась людям больше всего: ",
       "а вы знали про забавное клеше в фильмах: "]
...
mid = ["у меня отвалился хер ","я смотрел на него он смотрел на мой член ","бывало? ",",а один и говорит 'я гей' ","после этого ","дело было непростое ","была непростая дилема ","в перестрелке погибло трое ",
       ...
       "ясен пенис что ","тупые школьники ","скажем так ","на фоне был флаг украины ","хуй упал в краску ","федералы не смогли это остановить ","я ебал меня сосали "]
...
niggawork = ["иди нахуй","ты опять выходишь на связь, мудила?","отъебись","жопа"]
...
TOKEN = "ODMyMTk1Nzk1MDQyMTcyOTY5.YHgQug.7B_PEpwQ-z06ohPRRB4xxFBuN50"
...
async def шутка(ctx):
    ...
    if type == 1:
        joke_type = random.choice(beg)+random.choice(mid)+random.choice(other)+"\n - "+random.choice(dia)+"\n - "+random.choice(dia)+"\n - "+random.choice(dia)+"\n - "+random.choice(dia)+"\n "+random.choice(other)+random.choice(punch)
    if type == 2:
        joke_type = random.choice(beg)+random.choice(mid)+random.choice(other)+random.choice(other)+random.choice(mid)+random.choice(punch)
    if type == 3:
        joke_type = random.choice(beg)+random.choice(mid)+random.choice(mid)+random.choice(other)+random.choice(other)+random.choice(mid)+random.choice(punch)
    if type == 4:
        joke_type = random.choice(niggawork)
